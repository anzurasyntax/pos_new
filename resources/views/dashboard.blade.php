<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                @if ($isSalesUser)
                    <p class="mt-1 text-sm text-slate-500">Create estimates and convert them to sales when the customer is ready to pay.</p>
                @else
            ...
            @if ($canSell)
                <button
                    type="button"
                    onclick="window.dispatchEvent(new CustomEvent('open-estimate-wizard'))"
                    class="inline-flex items-center justify-center gap-2 rounded-xl bg-emerald-600 px-5 py-3 text-sm font-bold text-white shadow-sm hover:bg-emerald-700 transition shrink-0"
                >
    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
            @if ($isSalesUser)
                <div class="rounded-2xl bg-white p-8 shadow-soft ring-1 ring-slate-200/60 text-center">
                    <p class="text-slate-600 text-sm max-w-md mx-auto leading-relaxed">
                        Use <span class="font-semibold text-slate-800">Add Estimate</span> above to pick a customer, add products with quantities and prices, review the estimate, then convert it to a sale with payment details.
                    </p>
                </div>
            @else
    ...
    @if ($canSell && $estimateWizard)
        @include('partials.estimate-wizard', ['estimateWizard' => $estimateWizard])
    @endif
</x-app-layout>
